...
for %%f in ("%1\*.ply") do (
	...
	for %%a in (%CUDA_ALGORITHMS%) do (
		...
		REM %OPROC%  --algorithm="%%a" --in="%%f" --out="%DIR_OUT%\%%~nf-%%a-(auto).ply" --time-log="%DIR_OUT%\%%~nf-%%a-(auto).log" --runs=%RUNS%
